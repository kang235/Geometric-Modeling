...
public:
	...
	//binary operators
	const Vect4d operator+(const Vect4d & rhs) const
	{	
	...
	const Vect4d operator*(const float rhs) const
	{	
	...
	const friend Vect4d operator*(float scaleFactor, const Vect4d & rhs);

	const bool operator==(const Vect4d & rhs) const;
	const bool operator!=(const Vect4d & rhs) const
	{	return !((*this)==rhs);	}

	//self-add etc
	const void operator+=(const Vect4d & rhs)
	{	
	...
	//unary operators
	const Vect4d operator-(void) const {return Vect4d(-v[0],-v[1],-v[2],-v[3]);}
	const Vect4d operator+(void) const {return (*this);}
