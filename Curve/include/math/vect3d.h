...
public:
	...
	float AreaOfATriangle(Vect3d &b);
	...
	//linear interpolate
	const Vect3d lerp(const Vect3d & v2, float factor) const
	{return (*this)*(1.0f-factor)+v2*factor;}

	const Vect3d QuadraticInterpolate(const Vect3d & v2, const Vect3d & v3, float factor) const
	{return (*this)*(1.0f-factor)*(1.0f-factor)+2*v2*factor*(1.0f-factor)+v3*factor*factor;}

	//overloaded operators
	//binary operators
	const Vect3d operator+(const Vect3d & rhs) const 
	{return Vect3d(v[0]+rhs.v[0],v[1]+rhs.v[1],v[2]+rhs.v[2]);}

	const Vect3d operator+(const float rhs) const 
	{return Vect3d(v[0]+rhs,v[1]+rhs,v[2]+rhs);}
	
	const Vect3d operator-(const Vect3d &rhs) const   //vect-vect
	{return Vect3d(v[0]-rhs.v[0],v[1]-rhs.v[1],v[2]-rhs.v[2]); }

	const Vect3d operator-(const float rhs) const		//vect-float
	{return Vect3d(v[0]-rhs,v[1]-rhs,v[2]-rhs); }
	...
	{return Vect3d(v[0]*rhs,v[1]*rhs,v[2]*rhs);}
	const Vect3d operator*(const Vect3d &rhs) const
	{return Vect3d(v[0]*rhs.x(),v[1]*rhs.y(),v[2]*rhs.z());}
	...
	const friend Vect3d operator*(float scaleFactor, const Vect3d & rhs);
	...
	const bool operator==(const Vect3d & rhs) const;
	const bool operator!=(const Vect3d & rhs) const
	{	return !((*this)==rhs);	}

	//self-add etc
	const void operator+=(const Vect3d & rhs)
	{	
		v[0]+=rhs.v[0];	
		v[1]+=rhs.v[1];
		v[2]+=rhs.v[2];
	...
	const void operator*=(const float rhs)
	{	
		v[0]*=rhs;
		v[1]*=rhs;
		v[2]*=rhs;	
	...
	//unary operators
	const Vect3d operator-(void) const {return Vect3d(-v[0], -v[1], -v[2]);}
	const Vect3d operator+(void) const {return *this;}
